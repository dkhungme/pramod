...
public:
	...
	void Sort(char *input, char *output, int record_size){
...
public:
	void generate(char *name, int record_no, int record_size){
		FILE *fp;

		fp = fopen(name, "w+");
		char * str =  static_cast<char*>(malloc (record_size));
		int i;
		for(i =0; i<record_no; i++){
			gen_random(str, record_size);
			fwrite(str , 1 , record_size , fp );
		}
